...
;~ Global $g_aDefault[0] = "10000" ;Vermelho
;~ Global $g_aDefault[1] = "10000" ;Verde
;~ Global $g_aDefault[2] = "10000" ;Azul
;~ Global $g_aDefault[3] = "0" ;Preto
;~ Global $g_aDefault[4] = "10000" ;Branco
;~ Global $g_aDefault[5] = "0" ;Contraste
;~ Global $g_aDefault[6] = "0" ;Brilho
;~ Global $g_aDefault[7] = "0" ;Preenchimento de cor
;~ Global $g_aDefault[8] = "0" ;cor
;~ Global $g_aDefault[9] = "BitAnd($CA_NEGATIVE, $CA_LOG_FILTER)" ;filtro negativo
;~ Global $g_aDefault[10] = "0"


Global $g_aAdjust = $g_aDefault
$import_imagens = "\\192.168.1.34\g\GAFIC\images-negative"
...
While 1
			...
			GLOBAL $sFile = _FileListToArray($file, '*.png',"", true)
			local $last_fic = $sFile[0]
			;_ArrayDisplay($sFile, "array")

			ConsoleWrite("! last_fic: "&$last_fic&@CRLF&@CRLF)
			For $p = 1 to $last_fic Step 1
				If FileExists ($sFile[$p]) Then
					ConsoleWrite("! filep: "&$sFile[$p]&@CRLF&@CRLF)
					Global $sFile2 = $sFile[$p]
					global $sData = $sFile2
					ConsoleWrite("! p: "&$p&", arquivo; "&$sData&@CRLF)
					If $sFile[0] > 0 Then
						ConsoleWrite("> arquivo: "&$sData&", carregado"&@CRLF)
						$hObj = _GDIPlus_ImageLoadFromFile($sFile2)
						 If $hObj Then
						...
						Global $g_hTool = GUICreate('Adjustments', 303, 484, 10, 10, BitOR($WS_CAPTION, $WS_POPUP), 0, $hForm)
						...
						If $hObj Then
							If _GDIPlus_ImageSaveToFile($hObj, $sData) Then
								ConsoleWrite("- arquivo: "&$sData&", gravado!"&@CRLF)
								;_ArrayDelete($sFile, $p)

							EndIf
							_GDIPlus_ImageDispose($hObj)
						EndIf
					Else
						ConsoleWrite("!Tamanho da array <= 0")
						ExitLoop

					EndIf
				Else

					ConsoleWrite("!Nao existe o arquivo para o p:"&$p)
					ExitLoop
    ...
    ;$sData = FileOpenDialog('Load Image', @ScriptDir & '\Extras', 'Image Files (*.bmp;*.dib;*.gif;*.jpg;*.tif;*.png)|All Files (*.*)', 1 + 2)
    ;If @error Then Exit
...
While 1
    Switch GUIGetMsg()
        ...
        Case $aidButton[1]
            ...
            $sData = FileSaveDialog('Save Image', @WorkingDir, 'Image Files (*.bmp;*.dib;*.gif;*.jpg;*.tif;*.png)|All Files (*.*)', 2 + 16, @ScriptDir & '\MyImage.jpg', $g_hTool)
